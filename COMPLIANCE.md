...
## EXEC, GROM, and the ECS ROM

`exec.bin` (the Intellivision's system ROM / EXEC), `grom.bin` (the Graphics
ROM / GROM), and `ecs.bin` (the Entertainment Computer System's cartridge ROM,
needed only when ECS support is turned on in Settings) are Mattel Electronics
copyrighted firmware. They are **not** freely licensed and this project does
not have redistribution rights to them.

- `-DWITH_INTV_ROMS=ON` (the default for a local development build) embeds
  all three images from `tools/jzintv/roms/` into the binary via
  ...
  extra setup, and ECS can be enabled from Settings with no further steps.
  **A binary built this way must not be redistributed.**
- `-DWITH_INTV_ROMS=OFF` (what every artifact this repository publishes is
  built with) embeds none of the three images. The app reads them from the
  user's own ROM directory at run time, provisioned through the "Import
  System ROMs..." path once a milestone implementing it lands; without an
  imported `ecs.bin`, the ECS option in Settings is disabled.
  `core/tests/no_embedded_roms.py` is registered as a ctest against every
  built frontend executable (plus `boot_smoke`, once it exists) and fails the
  build if a distinctive slice of any of the three images is found in the
  shipped binary -- see that script's own header for how the check works and
  why it scans the whole file rather than a fixed offset.
