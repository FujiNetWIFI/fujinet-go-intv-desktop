...
#
# This is what makes the desktop app self-contained: the emulator connects
# out to a real FujiNet over BoIP (Bus Over IP) on loopback -- see
# core/jzintv/intv_host.c and src/fujinet/fn_sock.c in the staged jzIntv
# tree. Note the transport direction: FujiNet *listens* here (the RS232
# bus's BoIPChannel defaults to listening=true) and jzIntv connects out, the
# same direction fujinet-go-coco-desktop uses and the opposite of the ADAM
# and Apple II ports. The build does it by itself -- the sources are
# provided like any other dependency (cmake/Dependencies.cmake) and
# tools/fujinet/build-fujinet-desktop.sh stages, patches and builds them
# into tools/fujinet/work/out, where both an uninstalled run and the install
# rules pick it up. Modelled directly on fujinet-go-coco-desktop's own
# cmake/FujiNetRuntime.cmake.
#
# The first build takes several minutes (mbedTLS if no usable system copy is
...
if(WITH_FUJINET OR EXISTS "${FUJINET_LIB}")
  if(WIN32)
    ...
    install(FILES "${FUJINET_LIB}"
            DESTINATION ${CMAKE_INSTALL_LIBDIR}/fujinet-go-intv)
    install(FILES "${FUJINET_OUT}/fnconfig.ini"
            DESTINATION ${CMAKE_INSTALL_DATADIR}/fujinet-go-intv/fujinet)
    install(DIRECTORY "${FUJINET_OUT}/data" "${FUJINET_OUT}/SD"
            DESTINATION ${CMAKE_INSTALL_DATADIR}/fujinet-go-intv/fujinet)
  endif()
elseif(NOT WIN32)
