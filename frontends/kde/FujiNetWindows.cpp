...
void fujinet_config_show(QWidget *parent, intvsession *session)
{
#ifdef HAVE_WEBENGINE
    ...
    view->load(QUrl(QString::fromUtf8(intvsession_fujinet_webui_url(session))));
    ...
    QDesktopServices::openUrl(
        QUrl(QString::fromUtf8(intvsession_fujinet_webui_url(session))));
#endif
}

void fujinet_log_show(QWidget *parent, intvsession *session)
{
    static QPointer<QWidget> win;
    if (win) {
        win->raise();
    ...
    auto refresh = [view, session]() {
        ...
        const int n = intvsession_fujinet_copy_log(session, buf, sizeof(buf));
