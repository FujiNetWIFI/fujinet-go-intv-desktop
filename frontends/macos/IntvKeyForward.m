...
void IntvForwardKeyEvent(intvsession *session, NSEvent *event, int down)
{
     ...
     * unaffected (IntvForwardEcsKeyEvent below calls intvsession_ecs_key_set
     * directly, ignoring keyboard_mode). -keyDown: already drops
     * event.isARepeat before calling here, so no extra repeat guard is
     * needed for the fire-once behaviour a sysaction wants. */
