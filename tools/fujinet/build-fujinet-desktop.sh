#!/usr/bin/env bash
#
# Build the FujiNet RS232 runtime as a native Linux shared library
# (libfujinet.so) plus its default runtime assets (fnconfig.ini / data / SD)
# for the Intellivision desktop app.
#
# Adapted directly from fujinet-go-coco-desktop's own
# tools/fujinet/build-fujinet-desktop.sh; the desktop-embedding machinery
# below (staging, the FUJINET_EMBEDDED patches, mbedTLS resolution) is
# entirely target-agnostic and copied close to verbatim. What differs is
# PC_TARGET=RS232 (FUJINET_TARGET=RS232 in fujinet_pc.cmake -- BUILD_RS232,
# board fujinet-lwm-rs232, bus RS232) and the BOIP port forced below.
#
...
# (INTVSESSION_BOIP_PORT). Note what that means for THIS target: the RS232
# bus's BoIPChannel defaults to listening=true (see
# lib/hardware/BoIPChannel.cpp in the firmware tree -- rs232.cpp's own
# BoIPConfig().hostName(...).portNum(...) chain never overrides it), and the
# in-process jzIntv (src/fujinet/fn_sock.c in the staged jzIntv tree)
# connects out to it as a plain TCP client. Same direction as the CoCo
# port's DriveWire/Becker link, and the opposite of the ADAM and Apple II
# targets, where FujiNet is the client.
#
...
# jzIntv has no chdir() of its own today, but the rule below is what keeps
# that from mattering. Root it from the FUJINET_RUNTIME_ROOT env var the entry wrapper
...
# --- portability gaps in the RS232 device tree -----------------------------
...
# compiled for MinGW before its desktop port did it. lib/device/rs232 and
# lib/bus/rs232 have not either, so expect a FRESH set when the Windows build
# is first attempted -- the other targets' patches will not be the right
...
patch("version_common.py", [
    ...
    (
        ...
        '  """[fujinet-go-coco-desktop] git-optional variant: the desktop build\n'
...
patch("components_pc/libssh/cmake/Modules/FindMbedTLS.cmake", [
    (
        ...
        '# [fujinet-go-coco-desktop] Resolve a caller-pinned mbedTLS directly;\n'
...
TARGET_KEY_FILE="${CLONE_DIR}/build/.intv-target"
...
echo "FujiNet Intellivision desktop runtime outputs:"
